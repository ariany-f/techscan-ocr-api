...
namespace Tecno\Database {
    ...
    class Mysql implements Dbs {
        ...
        public function startConn()
        {
            ...
            {
                try {
                    $dsn = 'mysql:host=' . $this->config['host'] . ';port=' . $this->config['port'] . ';dbname=' . $this->config['database'] . ';charset=' . $this->config['encoding'];
                    ...
                    $connect->exec('SET NAMES ' . $this->config['encoding']);
                    $connect->exec('SET lc_time_names="' . $this->config['language'] . '"');
