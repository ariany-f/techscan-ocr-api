        ...
        public function getStatistics($id = null, $data_inicial = null, $data_final = null){
            ...
            try {
                    ...
                    $sql = "SELECT
                        ...
                        sum(case when preset_reason IS NULL AND description_reason IS NULL AND (updated_at IS NULL AND is_ok = 0) then 1 else 0 end) AS Acertos,
                        sum(case when preset_reason IS NOT NULL OR description_reason IS NOT NULL OR (updated_at IS NOT NULL AND is_ok <> 1) then 1 else 0 end) AS Erros
                    FROM passages
